...
async function fetchIndonesiaData(): Promise<{ entries: IndonesiaMacroEntry[]; biRate: { value: string; date: string } | null }> {
  ...
  const fredSeries: Array<{ id: string; title: string; unit: string; category: string; transform: (v: number) => string }> = [
    ...
    { id: 'IRSTCB01IDM156N', title: 'Indonesia BI Rate (Central Bank Rate)', unit: '%', category: 'rates', transform: (v) => `${v.toFixed(2)}%` },
    { id: 'IDNGDPNQDSMEI', title: 'Indonesia GDP (Quarterly)', unit: 'IDR T', category: 'growth', transform: (v) => `${(v / 1e12).toFixed(0)}T` },
  ]
