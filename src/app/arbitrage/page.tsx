      ...
      <div className="p-4 space-y-4 max-w-7xl mx-auto">
        <div className="flex items-end justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs font-mono text-teal-vivid">{summary.total} opportunities</span>
        ...
        <div className="flex items-center gap-1">
          {(['all', 'CEX Spot-Futures', 'DEX-CEX', 'Funding Arb'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-3 py-1 text-xs font-mono rounded transition-colors ${filter === f ? 'bg-teal-vivid text-bg-base font-bold' : 'text-text-muted hover:text-text-primary bg-bg-raised'}`}>
              {f === 'all' ? 'ALL' : f}
            </button>
          ))}
        </div>
            ...
            <table className="w-full border-separate border-spacing-0">
              <thead>
                <tr className="text-text-muted">
                  <SortableTh controls={tc} k="type" className="text-xs font-mono uppercase px-3 py-2 border-b border-bg-border text-left">Type</SortableTh>
                  ...
                  <SortableTh controls={tc} k="buyAt" className="text-xs font-mono uppercase px-3 py-2 border-b border-bg-border text-left">Buy At</SortableTh>
                  <SortableTh controls={tc} k="buyPrice" className="text-xs font-mono uppercase px-3 py-2 border-b border-bg-border text-right">Buy Price</SortableTh>
                  <SortableTh controls={tc} k="sellAt" className="text-xs font-mono uppercase px-3 py-2 border-b border-bg-border text-left">Sell At</SortableTh>
                  <SortableTh controls={tc} k="sellPrice" className="text-xs font-mono uppercase px-3 py-2 border-b border-bg-border text-right">Sell Price</SortableTh>
                  <SortableTh controls={tc} k="spread" className="text-xs font-mono uppercase px-3 py-2 border-b border-bg-border text-right">Spread</SortableTh>
                  <SortableTh controls={tc} k="signal" className="text-xs font-mono uppercase px-3 py-2 border-b border-bg-border text-center">Signal</SortableTh>
                </tr>
              </thead>
              <tbody>
                {tc.visible.map((o, i) => (
                  <tr key={`${o.symbol}-${o.type}-${i}`} className="border-b border-bg-border/30 hover:bg-bg-raised transition-colors">
                    ...
                    <td className="text-xs font-mono px-3 py-1.5 text-data-bull">{o.buyAt}</td>
                    <td className="text-xs font-mono px-3 py-1.5 text-right text-data-bull tabular-nums">${o.buyPrice.toLocaleString(undefined, { maximumFractionDigits: 6 })}</td>
                    <td className="text-xs font-mono px-3 py-1.5 text-data-bear">{o.sellAt}</td>
                    <td className="text-xs font-mono px-3 py-1.5 text-right text-data-bear tabular-nums">${o.sellPrice.toLocaleString(undefined, { maximumFractionDigits: 6 })}</td>
                    <td className="text-xs font-mono px-3 py-1.5 text-right font-bold text-text-primary tabular-nums">
                      {o.spreadBps > 0 ? `${o.spreadBps.toFixed(1)} bps` : o.spreadPercent > 0 ? `${o.spreadPercent.toFixed(2)}%` : '—'}
                    </td>
                    <td className="px-3 py-1.5 text-center">
                      <span className="text-xs font-mono font-bold text-accent-amber">{o.signal}</span>
                    </td>
                  </tr>
                ))}
                {tc.visible.length === 0 && (
                  <tr>
                    <td colSpan={8} className="text-center py-8 text-text-muted text-xs font-mono">
...
function CrossExFundingPanel() {
  ...
  return (
    <Panel title="CrossEx Funding Arbitrage" subtitle={meta ? `${meta.captured ?? rows.length} coins · Gate snapshot ${meta.ageMinutes}m ago` : 'Gate CrossEx aggregated funding rates'} liveStatus={meta ? (meta.stale ? 'stale' : 'live') : 'error'}>
      {error ? (
        <div className="p-4 text-xs font-mono text-text-muted text-center">{error}</div>
      ) : (
        <>
          ...
          <div className="overflow-auto scrollbar-thin">
            <table className="w-full border-separate border-spacing-0">
              <thead>
                <tr className="text-text-muted">
                  <SortableTh controls={tc} k="base" className="text-xs font-mono uppercase px-3 py-2 border-b border-bg-border text-left">Asset</SortableTh>
                  <SortableTh controls={tc} k="maxApy" className="text-xs font-mono uppercase px-3 py-2 border-b border-bg-border text-right">Max APR</SortableTh>
                  {CROSSEX_EXCHANGES.map(ex => (
                    <SortableTh key={ex} controls={tc} k={ex} className="text-xs font-mono uppercase px-3 py-2 border-b border-bg-border text-right">{ex}</SortableTh>
                  ))}
                </tr>
              </thead>
              <tbody>
                {tc.visible.map((r, i) => (
                  <tr key={`${r.base}-${i}`} className="border-b border-bg-border/30 hover:bg-bg-raised transition-colors">
                    ...
                    <td className="text-xs font-mono px-3 py-1.5 text-right font-bold text-data-bull tabular-nums">+{Number(r.maxApy).toFixed(2)}%</td>
                    {CROSSEX_EXCHANGES.map(ex => {
                      const v = r[ex]
                      const num = typeof v === 'string' ? Number(v) : NaN
                      return (
                        <td key={ex} className={`text-xs font-mono px-3 py-1.5 text-right tabular-nums ${num > 0 ? 'text-data-bull' : num < 0 ? 'text-data-bear' : 'text-text-muted'}`}>
                          {v === '' || v == null ? '—' : `${num > 0 ? '+' : ''}${num}%`}
                        </td>
                      )
                    })}
                ...
                {tc.visible.length === 0 && (
                  <tr>
                    <td colSpan={CROSSEX_EXCHANGES.length + 2} className="text-center py-8 text-text-muted text-xs font-mono">No assets match the filter.</td>
                  </tr>
                )}
              </tbody>
            </table>
...
// ─── CrossEx Futures Spread Arbitrage (per-exchange futures prices) ───

function CrossExSpreadPanel() {
  const [rows, setRows] = useState<CrossexRow[]>([])
  const [meta, setMeta] = useState<CrossexMeta | null>(null)
  const [error, setError] = useState<string | null>(null)
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const j = await res.json()
        if (!res.ok || !j.data) throw new Error(j.error ?? `HTTP ${res.status}`)
        const raw = (j.data.priceSpreadRate?.rows ?? []) as Array<{ base: string; maxApy: string; values: CrossexValue[] }>
  ...
  const tc = useTableControls(rows, undefined, { initialSortKey: 'maxApy', initialSortDir: 'desc' })
  return (

    <Panel title="CrossEx Futures Spread Arbitrage" subtitle={meta ? `${rows.length} coins · 14d convergence · Gate snapshot ${meta.ageMinutes}m ago` : 'Per-exchange futures price spreads'} liveStatus={meta ? (meta.stale ? 'stale' : 'live') : 'error'}>
      {error ? (
        <div className="p-4 text-xs font-mono text-text-muted text-center">{error}</div>
      ) : (
        <>
          <TableControlsBar idPrefix="crossex-spread" query={tc.query} onQueryChange={tc.setQuery} shown={tc.visible.length} total={tc.total} placeholder="Filter asset or exchange price…" />
          <div className="overflow-auto scrollbar-thin">
            <table className="w-full border-separate border-spacing-0">
              <thead>
                <tr className="text-text-muted">
              ...
              <tbody>
                {tc.visible.map((r, i) => (
                  <tr key={`${r.base}-${i}`} className="border-b border-bg-border/30 hover:bg-bg-raised transition-colors">
                    <td className="text-[12px] font-mono px-3 py-1.5 font-bold text-text-primary">{r.base}</td>
                    <td className="text-xs font-mono px-3 py-1.5 text-right font-bold text-accent-amber tabular-nums">+{(Number(r.maxApy) * 100).toFixed(2)}%</td>
                    {CROSSEX_EXCHANGES.map(ex => {
                      const v = r[ex]
                      return (
                        <td key={ex} className="text-xs font-mono px-3 py-1.5 text-right text-text-primary tabular-nums">
                          {v === '' || v == null ? '—' : String(v)}
                        </td>
                      )
                    })}
