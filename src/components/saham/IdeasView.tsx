...
export default function IdeasView({ externalQuery }: { externalQuery?: string }) {
  ...
  const filtered = useMemo(() => {
    ...
    const combined = ((externalQuery ?? '') + ' ' + search).trim()
    if (combined) {
    ...
    return ideas
  }, [data, minScore, sector, search, externalQuery, sortField, sortDir])
